...
namespace Api.Controllers
{
    public class FishCatchController : CustomBaseController
    {
        ...
        [HttpGet("[action]")]
        public async Task<ActionResult<List<FishCatchDto>>> GetPublicFishCatches([FromQuery] string pageSize, string pageIndex)
        {
        ...
        public async Task<ActionResult<List<FishCatchDto>>> GetFishCatchesByUserId(int userId, [FromQuery] string pageSize, [FromQuery] string pageIndex,[FromQuery] string sortBy)
        {
            var pageSizeNumber = int.Parse(pageSize);
            var pageIndexNumber = int.Parse(pageIndex);
            var totalFishCatches = await _fishCatchRepository.CountUserCatchesAsync(userId);
            var fishCatches = await _fishCatchRepository.GetFishCatchesByUserIdAsync(userId, pageSizeNumber, pageIndexNumber, sortBy);
            if (fishCatches.Count <= 0) return NoContent();
            return Ok(new
            {
                fishCatches,
